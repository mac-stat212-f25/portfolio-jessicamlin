...
SELECT title, score
FROM Posts
WHERE score >= 110 AND PostTypeID = 1
```
...
SELECT title 
FROM Posts
WHERE title LIKE ('%Berlin%') AND PostTypeId = 1
```
...
Id, Title, Score, Body, Tags
FROM Posts
WHERE Tags LIKE '%germany%' AND Tags LIKE '%walking%' 
ORDER BY Score DESC
```
...
```{r}
SELECT Tags, COUNT(Tags) numTagsets
FROM posts
GROUP BY Tags
ORDER BY  numTagsets
```
...
```{r}
SELECT TOP (100)
Text, Score, DisplayName, Reputation, Views, AboutMe
FROM Comments c
JOIN Users u
ON c.UserID = u.ID
ORDER BY Score Desc
