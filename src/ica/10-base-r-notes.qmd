...
fit_mod_and_extract <- function(data, yvar, preds, pred_of_interest) {
  # build formula string like "mpg ~ hp + wt"
  ...
  # coefficient estimate for predictor of interest
  est <- coef(mod)[pred_of_interest]

  # return tibble with estimate + CI
  tibble(
    which_pred = pred_of_interest,
    estimate = unname(est),
    ci_lower = ci[pred_of_interest, 1],
    ci_upper = ci[pred_of_interest, 2]
  )
}
```
