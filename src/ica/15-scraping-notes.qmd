...
```{r}
get_text_pg <- function(page, css_selector) {
  page |> 
...
scrape_page <- function(url) {
  ...
  article_titles <- get_text_pg(page, ".thumbnail-teaser__heading")
  article_dates_abstracts <- get_text_pg(page, ".thumbnail-teaser__description")

  tibble(
    title = article_titles,
    dates_abstract = article_dates_abstracts
  ) |>
...
for (i in 1:5) {
  ...
  vector[[i]] <- scrape_page(url)
}

df_articles <- bind_rows(vector)
head(df_articles)
```
