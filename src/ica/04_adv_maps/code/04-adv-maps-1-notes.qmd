...
mn_cities <- st_read("../data/shp_loc_pop_centers")
mn_water <- st_read("../data/shp_water_lakes_rivers")
```
...
transform_and_plot(nc, new_crs = "EPSG:32133", "North Carolina")

# Example using Antarctica
transform_and_plot(nc, new_crs = "EPSG:27702", "North Carolina")

# Example using North Dakota South
transform_and_plot(nc, new_crs = "EPSG:32121", "North Carolina")
```

**Verify your understanding:** If you had point location data that was not in the NAD27 CRS, what would you expect about the accuracy of how they would be overlaid on the original North Carolina map? I would expect it to be distorted.
...
st_crs(mn_water)
st_crs(mn_counties)
mn_counties <- mn_counties |> st_transform(st_crs(mn_cities))
```
...
```{r}
library(ggthemes)
ggplot() + geom_sf(data = mn_counties, fill = "white") + geom_sf(data = mn_cities, size = 0.5) + ggthemes::theme_map() + labs(title = "Minnesota")
```
...
- Read the function description at the top. What is the advantage of using this function for making color palettes? The viridis color scales provide maps that are uniform whether in color or black and white and is color-blind friendly.
- Look through the examples section. What is the difference between the `_d()`, `_c()`, and `_b()` variants of this function? The _d() variant is used when color is mapped to a discrete (categorical) variable. The _c() variant is used when color is mapped to a continuous variable. The _b() variant is used when color is mapped to a continuous variable but when we want that continuous variable to be binned so that there is a small set of colors.
...
```{r}
ggplot() + geom_raster(data = elev_df, aes(x = x, y = y, fill = elevation)) + scale_fill_gradient(low = "darkgreen", high = "white") + geom_sf(data = mn_counties, fill = "white") + 
    geom_sf(data = mn_cities |> filter(Population >= 10000), mapping = aes(color = Population, size = Population)) + # cities layer
