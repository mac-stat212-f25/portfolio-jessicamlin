...
for (i in seq_along(mtcars)) {
  col_classes[i] <- class(mtcars[[i]])[1]
}
# add names
names(col_classes) <- names(mtcars)
col_classes
```
...
```{r}
num_diamonds <- diamonds |> select(where(is.numeric))
stats_list <- map(num_diamonds, summ_stats)
stats_df <- bind_rows(stats_list) |>
  mutate(variable = names(num_diamonds), .before = 1)
stats_df
```
...
num_diamonds <- diamonds |> select(where(is.numeric))
stats_df_for <- tibble(
  variable = names(num_diamonds),
  mean = NA_real_,
  median = NA_real_,
  sd = NA_real_,
  iqr = NA_real_
)
for (i in seq_along(num_diamonds)) {
  x <- num_diamonds[[i]]
  stats_df_for$mean[i] <- mean(x, na.rm = TRUE)
  stats_df_for$median[i] <- median(x, na.rm = TRUE)
  stats_df_for$sd[i] <- sd(x, na.rm = TRUE)
  stats_df_for$iqr[i] <- IQR(x, na.rm = TRUE)
}
stats_df_for
```
...
```{r}
ex1 <- tibble(
  string = c("Macalester", "statistics", "purrr"),
  start  = c(1, 1, 2),
  end    = c(3, 4, 4)
)
pmap_chr(ex1, str_sub)
```
...
pmap_chr(ex2, str_sub, string = "unionization")
```
...
remove_outliers <- function(data, what_var, sd_thresh) {
  data |>
    mutate(
      zscore = ({{ what_var }} - mean({{ what_var }}, na.rm = TRUE)) /
        sd({{ what_var }}, na.rm = TRUE)
    ) |>
    filter(zscore <= sd_thresh)
}

fit_model <- function(data, mod_formula, do_remove_outliers) {
  ...
  lm(as.formula(mod_formula), data = data_clean)
}

# use pmap()
results <- df_arg_combos |>
  mutate(
    model = pmap(
      list(mod_formula = mod_formula, do_remove_outliers = remove_outliers),
      ~ fit_model(diamonds, mod_formula = ..1, do_remove_outliers = ..2)
    )
  )
