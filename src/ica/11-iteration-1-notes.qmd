...
```{r}    
# for loop
month_string <- vector("character", length = length(month.name))
...
diamonds |>
  dplyr::mutate(
    dplyr::across(.cols = c(x, y, z), .fns = ~ str_c(.x, " mm"))
  )
```
...
diamonds |>
  dplyr::mutate(
    dplyr::across(.cols = dplyr::where(is.numeric), .fns = as.character)
  )
```
...
```{r fit_model_example}
fit_model_example <- function(data, impute, filter_to) {
  if (impute) {
    data <- some_imputation_function(data)
  }

  if (filter_to == "Group 1") {
    data <- data |> dplyr::filter(group == 1)
  } else if (filter_to == "Groups 1 and 2") {
    data <- data |> dplyr::filter(group %in% c(1, 2))
  }
...
df_arg_combos <- tidyr::crossing(
  ...
  do_remove_outliers = c(TRUE, FALSE)
)
df_arg_combos
```
...
remove_outliers <- function(data, what_var, sd_thresh) {
  data |>
    mutate(
      z_score = ({{ what_var }} - mean({{ what_var }}, na.rm = TRUE)) /
        sd({{ what_var }}, na.rm = TRUE)
    ) |>
    filter(abs(z_score) <= sd_thresh) |>
    select(-z_score)
}
...
```{r}
vec_for_mod <- vector(mode = "list", length = nrow(df_arg_combos))

for (i in seq_along(vec_for_mod)) {
  this_formula <- df_arg_combos$mod_formula[[i]]
  this_do_remove_outliers <- df_arg_combos$do_remove_outliers[i]

  vec_for_mod[[i]] <- fit_model(
    data = diamonds,
    mod_formula = this_formula,
    do_remove_outliers = this_do_remove_outliers
  )
}
```
## Done!
