...
```{r}
convert_temp <- function(num, unit) {
  if (unit == "C") {
     new_val <- (num * 9/5) + 32
     str_glue("{new_val} F")
  } 
...
domain_name("jlin2@macalester.edu")
domain_name("jlin@gmail")
domain_name("jlin.com")
domain_name("jlin@gmail.")
```
...
```{r}
proportions <- function(df, count_var) {
  df |> 
    count(pick({{count_var}})) |> 
    mutate(prop = n/sum(n))
}
proportions(diamonds, cut)
proportions(diamonds, c(cut, color))
```
...
```{r}
scatter_smooth <- function(df, x, y) {
  df |> 
    ggplot(aes(x = {{x}}, y = {{y}})) + 
    geom_point() + 
